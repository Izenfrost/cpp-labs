...
public class Bullets implements Serializable {
	...
	static {
		bulletImages = new Image[] {
				tk.getImage("images/bulletL.gif"),

				tk.getImage("images/bulletU.gif"),

				tk.getImage("images/bulletR.gif"),

				tk.getImage("images/bulletD.gif"),
	...
	public boolean hitTank(Tank t) { 
		...
		if (this.live && this.getRect().intersects(t.getRect()) && t.isLive()
			...
			if (t.isGood()) {
				...
				if (t.getLife() <= 0) {
					tc.statistic.playerDeaths++;
					t.setLive(false);
				}
			} else {
				t.setLive(false);
				tc.enemyLifes --;
				tc.statistic.enemiesKilled++;
			}
	...
	public boolean hitHome() { 
		if (this.live && this.getRect().intersects(tc.home.getRect()) && !this.good) {
			this.live = false;
			this.tc.home.setLive(false);
			tc.statistic.homeDeaths++;
			return true;
		}
		return false;
	}
